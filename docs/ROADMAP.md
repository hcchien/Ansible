...
|---|---|---|---|
| Notification system — in-app feed + badge first, content-free push second | P1 — **✅ Phase A shipped 2026-06-13** (push = Phase B, open) | [Plan](superpowers/plans/2026-06-12-notification-system.md) | Local projection over already-synced data: replies/follows/messages notify with zero new server surface; feed + bell badge + per-category settings |
| Content reporting + board moderation tools | P1 — **✅ shipped 2026-06-13** (app tombstone rendering = follow-up) | [Plan](superpowers/plans/2026-06-12-content-reporting-moderation.md) | Reason-coded reports on both rails with tier-aware rate limits, web moderator console (queue/actions/audit), host-scoped tombstones + thread locks enforced at both write chokepoints |
...
| **Identity recovery & re-anchor design** — multi-device attestation, encrypted content-key backup, relay re-anchor protocol, anchor as user-signed portable object | P1 — launch blocker, **gates hardware custody** | [Architecture plan — Phase 1.0](architecture/service_architecture_plan.md) | Hardware custody makes keys unexportable, so device loss = identity loss unless recovery is designed first (constitution Base Rule 1). Decisions D1/D5 pending |
