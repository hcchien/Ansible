...
| Cross-cutting foundations — app↔relay API versioning + observability baseline | P1 — **API versioning ✅ done 2026-06-13**; observability baseline still open | [Architecture plan — Phase 0](architecture/service_architecture_plan.md) | Versioning shipped: `x-ansible-protocol` header + `/api/v1/meta` advertisement + `426 upgrade_required` enforcement on relay & appview, op `schema_version` (additive, kept out of signed payload). Observability (metrics endpoints) is the remaining Phase 0 item |
