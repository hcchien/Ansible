...
| Android release readiness (beta build → Play Store) | P2 | [Plan](superpowers/plans/2026-05-10-android-release-readiness-checklist.md) | **Rust core now ships in the APK (2026-07-03)** — cargokit wired into the DID plugin's Android gradle; debug APK builds with libansible_rust_core.so for arm64/armv7/x86_64. Remaining: on-device platform verification, signing keys + Play listing (user-held) |
...
| [Encrypted messenger protocol](superpowers/plans/2026-05-14-encrypted-messenger-protocol.md) · [hardening/scale](superpowers/plans/2026-09-01-messenger-product-and-gcp-scaling-plan.md) | [design](superpowers/specs/2026-05-14-encrypted-messenger-protocol-design.md) | 1:1 opaque messenger: Phase 1-2 transport and multi-device hardening landed 2026-09-01 (revocation, fan-out, cursor, TTL, idempotency, pre-key lifecycle, key-change fail-closed). Custom `signal-mvp-v1` crypto is not Signal-grade and remains blocked from production security claims pending audited-library replacement/review. Expansion remains contained. |
