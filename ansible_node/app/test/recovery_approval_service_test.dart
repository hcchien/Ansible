...
void main() {
  ...
  Future<void> seedActiveAnchor() async {
    final identityKey = InMemoryIdentityKey(
    ...
    final sig =
        await identityKey.sign(utf8.encode(unsigned.canonicalBodyJson()));
  ...
  RecoveryApprovalService service(
    MockClient client, {
    DeviceKey? approverKey,
  }) {
  ...
  test('full round trip: build → QR → parse → approve submits a valid proof',
      () async {
    ...
    // QR round trip.
    final parsed =
        RecoveryApprovalService.parseRequest(request.toQrPayload());
    ...
    // against the OLD device key over the canonical body — exactly what the
    // relay's veto/recovery verifier checks.
    ...
    expect(
      await Ed25519Keys.verify(
        publicKeyHex: oldDeviceKey.publicKeyHex,
  ...
  test('refuses a request for someone else\'s identity', () async {
    final client = relay();
    final request =
        await service(client).buildRequest(did: did, handle: handle);
    final oldDevice = service(client, approverKey: oldDeviceKey);

    await expectLater(
      oldDevice.approve(localDid: 'did:elix:notme', anchor: request!.anchor),
      throwsA(
        isA<RecoveryApprovalException>()
            .having((e) => e.reason, 'reason', 'different_identity'),
      ),
    );
  });

  test('refuses a stale request that does not chain onto the active anchor',
      () async {
    final client = relay();
    final request =
        await service(client).buildRequest(did: did, handle: handle);
    ...
    await expectLater(
      ...
      throwsA(
        isA<RecoveryApprovalException>()
            .having((e) => e.reason, 'reason', 'stale_request'),
      ),
    );
  });

  test('refuses to approve without an enrolled device key', () async {
    final client = relay();
    final request =
        await service(client).buildRequest(did: did, handle: handle);

    final oldDevice = service(client); // no device key
    await expectLater(
      oldDevice.approve(localDid: did, anchor: request!.anchor),
      throwsA(
        isA<RecoveryApprovalException>()
            .having((e) => e.reason, 'reason', 'no_device_key'),
      ),
    );
  });
