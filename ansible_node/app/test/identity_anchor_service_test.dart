        ...
        if (request.method == 'GET') {
          ...
          if (anchor == null) {
            return http.Response(
              jsonEncode({'error': 'anchor_not_found'}),
              404,
            );
          }
...
void main() {
  test('publishInitialAnchor emits schema v4 for a v1 registration', () async {
    ...
    final anchor = await repo.latest(did);
    expect(anchor!.schemaVersion, 4);
    expect(anchor.genesisCommitment, commitment);
  });

  test(
    'publishInitialAnchor builds + signs + persists + posts (201)',
    () async {
      final relay = _FakeRelay();
      final repo = InMemoryIdentityAnchorRepository();
      final service = IdentityAnchorService(
  ...
  test(
    'device-loss walkthrough: backup restore re-anchors via rotation',
    () async {
      final relay = _FakeRelay();
      ...
      final identityKey = await _newIdentityKey();
      final identityPrivateKeyHex =
          (identityKey as InMemoryIdentityKey).privateKeyHex;
      ...
      String?
      recoveredKeyInSecureStorage; // stands in for secure storage write.
      ...
      final previous = await serviceB.relayClient.fetchActiveAnchor(
        'did:plc:alice',
      );
  ...
  test(
    'recoverFromBackupRotation does NOT persist on relay rejection',
    () async {
      final relay = _FakeRelay();
      ...
      final previous = await service.relayClient.fetchActiveAnchor(
        'did:plc:alice',
      );
