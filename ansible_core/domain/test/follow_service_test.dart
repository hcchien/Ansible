...
void main() {
  group('FollowService', () {
    ...
    test('follow native DID creates pending federated request', () async {
      ...
      final edges = await followRepo.listOutbound('did:key:local');
      expect(edges.single.status, FollowStatus.pending);
      expect(edges.single.visibility, FollowVisibility.federated);
    });
    ...
    test(
      'unfollow purges the author\'s follow-only posts and content',
      () async {
        ...
        await contentRepo.create(
          citem('m-bob', 'did:key:bob'),
        ); // kept (other author)

        // Resolve alice's targetId to unfollow.
        final aliceEdge = (await followRepo.listOutbound(
          'did:key:local',
