defmodule AnsibleRelay.Db.DidElixMigration do
  ...
  schema "did_elix_migrations" do
    field(:v1_did, :string)
    field(:handle, :string)
    field(:state, :string, default: "completed")
  ...
  def changeset(struct, attrs) do
    ...
    |> cast(
      attrs,
      ~w(legacy_did v1_did handle state canonical_body legacy_sig v1_sig created_at completed_at)a
    )
    |> validate_required(
      ~w(legacy_did v1_did handle state canonical_body legacy_sig v1_sig created_at completed_at)a
    )
    |> validate_inclusion(:state, ~w(completed))
