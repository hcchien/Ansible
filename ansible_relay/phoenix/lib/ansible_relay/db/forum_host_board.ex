  ...
  defp validate_posting_policy(changeset) do
    validate_change(changeset, :posting_policy, fn :posting_policy, policy ->
      ...
      deliberationCreation =
        Map.get(policy, "deliberation_creation") || Map.get(policy, :deliberation_creation)

      if (is_nil(tier) or AnsibleRelay.ReputationTier.valid_min_post_tier?(tier)) and
           (is_nil(pollCreation) or pollCreation in ["posters", "moderators", "owners"]) and
           (is_nil(deliberationCreation) or
              deliberationCreation in ["posters", "moderators", "owners"]) do
        []
      else
